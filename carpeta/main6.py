...
class Nivel3(NivelBase):
    def __init__(self):
        ...
        # Nivel 3: Patrón más complejo con grupos de pinchos
        x = 350
        for i in range(20):
            # Crear grupos de 2-3 pinchos juntos
            if i % 5 == 0 or i % 5 == 1:
                # Grupo de pinchos juntos (más difícil)
                pinchos.append(pygame.Rect(x, Suelo_Y - 30, 28, 38))
                x += 180  # Muy juntos
            elif i % 5 == 2:
                # Pincho alto y grande
                pinchos.append(pygame.Rect(x, Suelo_Y - 40, 38, 50))
                x += 300
            else:
                # Pincho normal
                pinchos.append(pygame.Rect(x, Suelo_Y - 30, 30, 40))
        ...
        super().__init__(FONDO_NIVEL_3, pinchos)
        self.velocidad_nivel = 7  # ← Más rápido que los niveles anteriores
